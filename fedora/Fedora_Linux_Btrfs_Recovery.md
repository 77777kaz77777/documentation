...
```
3. Roll back to a snapshot number created *before* the issue occurred (e.g., #2):
```bash
...
```
2. Send and receive the snapshot (replace `ROOT.20260813T1431` with your actual backup folder name):
```bash
...
```
2. Backups transferred via `btrfs send/receive` are marked **read-only**. Remove the read-only flag so Fedora can boot and write to it normally:
```bash
...
```
2. Run `sudo btrbk run` to create a fresh baseline snapshot on your 1TB drive.
